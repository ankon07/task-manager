    ...
    .then(savedUser => {
      ...
      if (roles && roles.length > 0) {
        Role.find({ name: { $in: roles } }).exec()
          ...
          .catch(err => {
            // Rollback user creation if role assignment fails? Consider transaction logic for robustness.
            res.status(500).send({ message: err.message || "Error assigning roles." });
          });
      } else {
        ...
        Role.findOne({ name: "user" }).exec()
          ...
          .then(() => {
            // Send verification email [cite: 2]
            // emailUtil.sendVerificationEmail(savedUser.email, savedUser.emailVerificationToken);
            res.send({ message: "User registered successfully! Please check your email to verify your account." });
          })
          .catch(err => {
             res.status(500).send({ message: err.message || "Error assigning default role." });
          });
      }
    })
    .catch(err => {
